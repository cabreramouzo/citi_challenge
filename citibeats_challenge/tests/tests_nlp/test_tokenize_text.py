...
def test_text_tokenize_sample_en():
    text = "Agent Smith: As you can see, when've had our eye on you for some time now, Mr. Anderson."
    tkns = get_tokens(text=text, lang="en")
    res = ["Agent","Smith","As","you","can","see","when","have","had","our","eye","on","you","for","some","time","now","Mr.","Anderson"]
    print(tkns)
    print(res)
    assert( tkns == res)
